"""
Test Flash Attention unified interface - verify FA3/FA2/SDPA selection and SDPA correctness.
...
def set_impl(impl):
    """Set the implementation override ('fa3', 'fa2', 'sdpa', or None for auto) and re-resolve backend state."""
    fa_module._override_impl = impl
    fa_module.ATTENTION_BACKEND = fa_module._resolve_attention_backend()
    fa_module.USE_FA3 = fa_module.ATTENTION_BACKEND == 'fa3'
    fa_module.USE_FA2 = fa_module.ATTENTION_BACKEND == 'fa2'
...
class TestOverrideMechanism:
    ...
    def test_auto_backend_prefers_fa3_then_fa2_then_sdpa(self, monkeypatch):
        """Auto-selection should choose FA3 first, then FA2, then SDPA."""
        monkeypatch.setattr(fa_module, "_flash_attention_dtype_supported", lambda backend: True)
        monkeypatch.setattr(fa_module, "HAS_FA3", True)
        monkeypatch.setattr(fa_module, "HAS_FA2", True)
        fa_module._override_impl = None
        assert fa_module._resolve_attention_backend() == 'fa3'

        monkeypatch.setattr(fa_module, "HAS_FA3", False)
        assert fa_module._resolve_attention_backend() == 'fa2'

        monkeypatch.setattr(fa_module, "HAS_FA2", False)
        assert fa_module._resolve_attention_backend() == 'sdpa'

    def test_rocm_backend_skips_fa3_but_allows_fa2_bf16(self, monkeypatch):
        """ROCm GPUs such as MI300X should use FA2 when flash-attn is installed."""
        monkeypatch.setattr(fa_module, "_is_rocm", lambda: True)
        monkeypatch.setattr(fa_module.torch.cuda, "is_available", lambda: True)
        monkeypatch.setattr(fa_module, "HAS_FA3", False)
        monkeypatch.setattr(fa_module, "HAS_FA2", True)
        monkeypatch.setattr(fa_module, "_override_impl", None)
        monkeypatch.setattr(fa_module, "COMPUTE_DTYPE_FOR_TEST", torch.bfloat16, raising=False)
        monkeypatch.setattr(
            fa_module,
            "_get_compute_dtype",
            lambda: fa_module.COMPUTE_DTYPE_FOR_TEST,
        )

        assert fa_module._flash_attention_dtype_supported("fa2") is True
        assert fa_module._resolve_attention_backend() == "fa2"

    def test_flash_attn_func_dispatches_to_fa2_backend(self, monkeypatch):
        """Training attention should use FA2 when FA2 is the selected backend."""
        class FA2Stub:
        ...
        monkeypatch.setattr(fa_module, "_fa2", FA2Stub())
        monkeypatch.setattr(fa_module, "ATTENTION_BACKEND", "fa2")

        q = torch.zeros(1, 8, 2, 4)
        y = flash_attn.flash_attn_func(q, q, q, causal=True, window_size=(8, 0))

        assert torch.equal(y, torch.full_like(q, 2.0))
    ...
    def test_override_fa2(self, monkeypatch):
        """Test that override='fa2' uses FA2 when available."""
        monkeypatch.setattr(fa_module, "HAS_FA2", True)
        monkeypatch.setattr(fa_module, "_flash_attention_dtype_supported", lambda backend: True)
        set_impl('fa2')
        assert fa_module.USE_FA2 == True
        assert fa_module.ATTENTION_BACKEND == 'fa2'
        set_impl('sdpa')
    ...
    def test_override_auto(self):
        ...
        assert fa_module.USE_FA3 == (fa_module.ATTENTION_BACKEND == 'fa3')
        assert fa_module.USE_FA2 == (fa_module.ATTENTION_BACKEND == 'fa2')
...
if __name__ == "__main__":
    ...
    print(f"HAS_FA2: {fa_module.HAS_FA2}")
